...
async def showAdminPanel(message: types.Message):
    ...
    await message.answer(text="🔧⚙️ПАНЕЛЬ АДМИНИСТРАТОРА🔩🔌\n\n"
                              ...
                              "‼️ВНИМАНИЕ‼️\n"
                              "Каждое действие в режиме администратора имеет перманентный характер. "
                              "Выполняйте операцию, лишь четко понимая последствия",
...
async def callbackFlyControl(call: types.CallbackQuery):
    ...
    await call.message.answer(text="🚀ВХОД В ЦЕНТР УПРАВЛЕНИЯ ПОЛЕТОМ🚀\n"
                                   ...
                                   "Нажмите на кнопку 'Подсказка', а затем введите ключ, "
                                   "состоящий из 9 латинских символов ➡️",
                              reply_markup=keyboard)
...
async def processHeight(message: types.Message, state: FSMContext):
    ...
    if int(message.text) < 35000:
        await message.reply('‼ Слишком малая высота ‼\n'
                            'Угроза сваливания спутника. Введите высоту больше ➡')
        await Answers.heightKilometres.set()
        return
    elif int(message.text) > 45000:
        await message.reply('‼ Слишком большая высота ‼\n'
                            'Угроза потери контроля над станцией. Введите высоту меньше ➡')
        await Answers.heightKilometres.set()
        return
    else:
        hight = message.text
        await message.reply('Высота принята!\n'
                            'Начинаю корректировку орбиты 🎛')
        await asyncio.sleep(3)
        await message.answer('🚀 Выполняю рассчет угла наклона двигателей 🚀')
        await asyncio.sleep(2)
        await message.answer("💥 Перевржу двигатели в режим корректировки 💥")
        await asyncio.sleep(3)
        await message.answer('Корректировка орбиты начата! ✅\n'
                             f'Станция займет высоту {hight} км через 17 часов ⏰')
        await asyncio.sleep(2)
...
async def callbackCargoDumping(call: types.CallbackQuery):
    ...
    await call.message.answer(
        "Чтобы вернуть грузы отправителям, вам понадобятся номера их грузов. Они содержатся в файле с данными.\n"
        "Нажмите на кнопку 'Данные', изучите отправленный файл, а после введите номера грузов, "
...
async def processCargoDumping(message: types.Message, state: FSMContext):
    ...
    if message.text.isdigit() is False or int(message.text) not in cargoNumbers:
        await message.reply('Некорректный номер груза.\n'
                            'Проверьте правильность ввода и повторите отправку сообщения ➡')
        await Answers.cargoDumping.set()
        return
    cargoNumbers.remove(int(message.text))
    if len(cargoNumbers) != 0:
        await message.reply('Груз готовится к отправке!\n'
                            'Введите следующий номер ➡')
        await Answers.cargoDumping.set()
        return
    await message.answer('📦 Набор для доставки сформирован 📦\n'
                         'Следующая доставка будет доступна через 42 часа ✅')
    await asyncio.sleep(2)
    await showAdminPanel(message)
...
async def callbackAutopilot(call: types.CallbackQuery):
    ...
    await call.message.answer(f"🔐Для выполнения этого действия необходима верификация🔐\n"
                              f"\n"
                              r"Нажмите на кнопку 'Подсказка', а после введите свой ключ авторизации, "
                              r"состоящий из 9 латинских символов:",
                              reply_markup=keyboard)
...
async def processAutopilot(message: types.Message, state: FSMContext):
    await state.finish()
    if message.text.lower() != 'vancouver':
        await message.reply('‼ Ключ верификации неверен ‼\n'
                            'Попробуйте снова ➡️')
        await Answers.autoPilot.set()
        return
    await message.reply('‼ Станция переведена в режим автопилота ‼\n\n'
                        'Ручное управление отключено ❌\n'
                        '\n'
                        'Вы будете переведены в статус контролера: Вы не сможете осуществлять управление станцией, но будете оперативно получать отчеты о ее состоянии 👩🏼‍💼\n'
                        f'Спасибо за сотрудничество, мисс {message.from_user.first_name} 👋🏻')
    # Завершающая стадия квеста
    await asyncio.sleep(90)
    await message.answer('‼ТРЕВОГА‼')
    await asyncio.sleep(10)
    await message.answer('‼Ошибка при корректировки траектории‼\n'
                         'Для исправления будет сброшено несколько резервных грузовых отсеков 📦')
    await asyncio.sleep(6)
    await message.answer("Грузовой отсек 12392 сброшен 📦")
    await asyncio.sleep(15)
    await message.answer("Грузовой отсек 12568 сброшен 📦")
    await asyncio.sleep(10)
    await message.answer("Грузовой отсек 12121 сброшен 📦")
    await asyncio.sleep(25)
    await message.answer("Грузовой отсек 12000 сброшен 📦")
    await asyncio.sleep(10)
